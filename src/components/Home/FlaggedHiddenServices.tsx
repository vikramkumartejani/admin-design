...
const FlaggedHiddenServices = () => {
    const transactions = [
        {
            id: 1,
            title: 'Social Strategy Media',
            provider: 'Jane M.',
            status: 'Flagged',
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'Hidden':
                return 'bg-[#4545451A] text-[#454545]'
            default:
    ...
    return (
        <div className="bg-white rounded-[24px] border border-[#E8ECF4]">
            {/* Header */}
            <div className="flex items-center justify-between flex-wrap gap-3 mb-3.5 px-4 sm:px-6 pt-4">
                <h3 className="text-[20px] leading-[30px] sm:leading-[38px] tracking-[-1px] font-semibold text-[#252525]">
                    Flagged/Hidden Services
                </h3>
                <Link href='/services/flag-hide' className="bg-[#3A96AF] text-white px-4 py-2 rounded-lg text-[14px] font-medium hover:bg-[#2d7a8f] transition-colors duration-200">
                    View All
                </Link>
            </div>
            ...
            <div className="overflow-x-auto">
                <table className="min-w-[748px] w-full text-nowrap">
                    ...
                    <tbody>
                        {transactions.map((transaction) => (
                            <tr key={transaction.id} className="border-b border-[#F3F6F9] hover:bg-gray-50 transition-colors duration-150">
                                <td className="px-6 py-5 text-[14px] leading-[20px] font-medium text-[#252525] text-wrap w-[150px]">
                                    {transaction.title}
                                </td>
...
export default FlaggedHiddenServices